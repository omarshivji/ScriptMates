...
import {
  handleTryItOutDeployment,
  loadTryItOutDeployment,
} from "./localDeployment/tryitout.js";
  ...
  switch (projectSelection.kind) {
    case "deploymentName": {
      ...
      if (projectSelection.deploymentType === "tryItOut") {
        const credentials = await loadTryItOutDeployment(
          ctx,
          projectSelection.deploymentName,
        );
        return {
          deploymentName: projectSelection.deploymentName,
          adminKey: credentials.adminKey,
          url: localDeploymentUrl(credentials.ports.cloud),
