...
var import_tryitout = require("./lib/localDeployment/tryitout.js");
async function deploymentCredentialsOrConfigure(ctx, deploymentSelection, chosenConfiguration, cmdOptions, partitionId) {
...
async function _deploymentCredentialsOrConfigure(ctx, deploymentSelection, chosenConfiguration, cmdOptions, partitionId) {
  ...
  switch (deploymentSelection.kind) {
    ...
    case "tryItOut": {
      ...
      const result = await (0, import_tryitout.handleTryItOutDeployment)(ctx, {
        chosenConfiguration,
...
async function handleDeploymentWithinProject(ctx, {
  ...
  const hasAuth = ctx.bigBrainAuth() !== null;
  const loginMessage = hasAuth ? void 0 : `Tip: You can try out Convex without creating an account by clearing the ${import_utils.CONVEX_DEPLOYMENT_ENV_VAR_NAME} environment variable.`;
  await (0, import_login.ensureLoggedIn)(ctx, {
  ...
  if (accessResult.kind === "noAccess") {
    (0, import_context.logMessage)(ctx, "You don't have access to the selected project.");
    const result = await handleChooseProject(
      ctx,
      chosenConfiguration,
      {
