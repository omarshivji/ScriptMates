...
var import_tryitout = require("./localDeployment/tryitout.js");
...
const deploymentSelectionWithinProjectSchema = import_zod.z.discriminatedUnion(
  ...
  [
    ...
    import_zod.z.object({ kind: import_zod.z.literal("prod"), partitionId: import_zod.z.number().optional() }),
    import_zod.z.object({
      kind: import_zod.z.literal("implicitProd"),
      partitionId: import_zod.z.number().optional()
    }),
    import_zod.z.object({ kind: import_zod.z.literal("ownDev"), partitionId: import_zod.z.number().optional() })
  ]
);
async function deploymentSelectionWithinProjectFromOptions(ctx, options) {
...
async function fetchDeploymentCredentialsProvisioningDevOrProdMaybeThrows(ctx, projectSelection, deploymentType, partitionId) {
...
async function handleOwnDev(ctx, projectSelection, partitionId) {
  switch (projectSelection.kind) {
    case "deploymentName": {
      ...
      if (projectSelection.deploymentType === "tryItOut") {
        const credentials = await (0, import_tryitout.loadTryItOutDeployment)(
          ctx,
          projectSelection.deploymentName
        );
        return {
          deploymentName: projectSelection.deploymentName,
          adminKey: credentials.adminKey,
          url: (0, import_run.localDeploymentUrl)(credentials.ports.cloud),
...
async function loadSelectedDeploymentCredentials(ctx, deploymentSelection, selectionWithinProject, { ensureLocalRunning } = { ensureLocalRunning: true }) {
  switch (deploymentSelection.kind) {
    ...
    case "tryItOut": {
      ...
      const config = await (0, import_tryitout.loadTryItOutDeployment)(
        ctx,
        deploymentSelection.deploymentName
      );
