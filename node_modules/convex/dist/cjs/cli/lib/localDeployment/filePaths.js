...
function loadDeploymentConfig(ctx, deploymentKind, deploymentName) {
  ...
  if (ctx.fs.exists(configFile)) {
    ...
    try {
      ...
      (0, import_context.logVerbose)(ctx, `Failed to parse local deployment config: ${e}`);
      return null;
    }
  }
  return null;
...
function dashboardDir() {
  return import_path.default.join((0, import_utils.rootDirectory)(), "dashboard");
}
...
function loadDashboardConfig(ctx) {
  ...
  try {
    ...
    (0, import_context.logVerbose)(ctx, `Failed to parse dashboard config: ${e}`);
    return null;
  }
}
