...
var tryitout_exports = {};
__export(tryitout_exports, {
  handleTryItOutDeployment: () => handleTryItOutDeployment,
  listExistingTryItOutDeployments: () => listExistingTryItOutDeployments,
  loadTryItOutDeployment: () => loadTryItOutDeployment
});
module.exports = __toCommonJS(tryitout_exports);
...
async function handleTryItOutDeployment(ctx, options) {
  ...
  if (deployment.kind === "existing") {
    ...
    const data = await (0, import_bigBrain.bigBrainGenerateTryItOutAdminKey)(ctx, {
...
async function loadTryItOutDeployment(ctx, deploymentName) {
  const config = (0, import_filePaths.loadDeploymentConfig)(ctx, "tryItOut", deploymentName);
  if (config === null) {
    return ctx.crash({
      exitCode: 1,
      errorType: "fatal",
...
async function listExistingTryItOutDeployments(ctx) {
  const dir = (0, import_filePaths2.rootDeploymentStateDir)("tryItOut");
...
async function chooseTryItOutDeployment(ctx, options) {
...
async function promptForNewDeployment(ctx, existingNames) {
  ...
  const message = isFirstDeployment ? "Choose a name for your first deployment:" : "Choose a name:";
  const deploymentName = await (0, import_prompts.promptString)(ctx, {
    message,
    default: import_path.default.basename(process.cwd())
  });
  const uniqueName = await getUniqueName(ctx, deploymentName, existingNames);
  (0, import_context.logVerbose)(ctx, `Deployment name: ${uniqueName}`);
...
//# sourceMappingURL=tryitout.js.map
