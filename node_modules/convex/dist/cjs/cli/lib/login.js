...
async function performLogin(ctx, {
  ...
  let auth0;
  let accessToken;
  if (authFlow === "paste" || authFlow === "auto" && (0, import_utils.isWebContainer)()) {
  ...
  (0, import_context.logVerbose)(ctx, `performLogin: updating big brain auth after login`);
...
async function optins(ctx, acceptOptIns) {
  ...
  switch (bbAuth.kind) {
    ...
    case "previewDeployKey":
      return true;
    default: {
      const _exhaustivenessCheck = bbAuth;
      return await ctx.crash({
        exitCode: 1,
        errorType: "fatal",
...
async function ensureLoggedIn(ctx, options) {
  ...
  if (!isLoggedIn) {
    if (options?.message) {
      (0, import_context.logMessage)(ctx, options.message);
    }
