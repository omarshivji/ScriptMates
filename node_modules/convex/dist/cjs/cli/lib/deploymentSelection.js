...
function logDeploymentSelection(ctx, selection) {
  switch (selection.kind) {
    ...
    case "preview": {
      (0, import_context.logVerbose)(ctx, `Preview deploy key`);
      break;
    }
    case "chooseProject": {
      (0, import_context.logVerbose)(ctx, `Choose project`);
      break;
    }
    case "tryItOut": {
      (0, import_context.logVerbose)(ctx, `Try it out`);
      break;
    }
    default: {
      const _exhaustivenessCheck = selection;
      (0, import_context.logVerbose)(ctx, `Unknown deployment selection`);
    }
  }
  return null;
}
...
async function _getDeploymentSelection(ctx, cliArgs) {
  ...
  if (cliArgs.envFile) {
    (0, import_context.logVerbose)(ctx, `Checking env file: ${cliArgs.envFile}`);
...
async function getDeploymentSelectionFromEnv(ctx, getEnv) {
  ...
  if (deployKey !== null) {
    ...
    switch (deployKeyType) {
      ...
      default: {
        const _exhaustivenessCheck = deployKeyType;
        return ctx.crash({
          exitCode: 1,
          errorType: "fatal",
  ...
  if (convexDeployment !== null) {
    ...
    const isTryItOut = (0, import_deployment.isTryItOutDeployment)(targetDeploymentName);
    if (isTryItOut) {
...
async function checkIfBuildEnvironmentExpectsConvexDeployKey(ctx) {
